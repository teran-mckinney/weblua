...
-- Edited by sega01 for Zenserver default page.

require "gd"

function createClock(size, hours, minutes)
  local im = gd.createTrueColor(size, size)
  local gray = im:colorAllocate(49, 51, 55)
  local transp = im:colorAllocate(0, 0, 0, 0)
  local blue = im:colorAllocate(1, 51, 112)
  local cxy = size/2

  im:filledRectangle(0, 0, size, size, transp)
  im:setThickness(math.max(1, size/100))
  im:arc(cxy, cxy, size, size, 0, 360, blue)
...
print("Pragma: no-store")       -- Can mozilla understand this?
